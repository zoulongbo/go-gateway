...
func NewRedisFlowCountService(appID string, interval time.Duration) (*RedisFlowCountService, error) {
	...
	go func() {
		...
		for {
			...
			curr := time.Now()
			dayKey := reqCounter.GetDayKey(curr)
			hourKey := reqCounter.GetHourKey(curr)
			if err := RedisConfPipline(func(c redis.Conn) {
				...
				fmt.Println("redis flow pipline err:", err)
				continue
			}

			totalCount, err := reqCounter.GetDayData(curr)
			if err != nil {
				fmt.Println("redis flow GetDayData err:", err)
				continue
			}
